...
<body onload="loadFriends()">
	...
	<ul class="topnav">
		...
		<li><a href="conv_page.jsp" title="Conversations"><i class="fas fa-comment icon"></i>Conversations</a></li>
		...
		function extract(array) {
			...
			if(array.length > 0){
				for(var i=0; i<array.length; i++){
					...
					//out += "<a href='#' id='result" + i + "' onclick='reDirect(" + array[i].user_id + ")'>" + array[i].first_Name + " " + array[i].last_Name + "</a>";
				}
				var x = document.getElementById("hint");
		...
		function loadFriends(){
			...
			for(var i=0; i<str.length; i++){
				out += "<div class='card'><img class='pr_img' src='" + ph[i] + "' alt='"+ str[i].first_Name +
					...
					"<a id='lnk2' href='#'>Send message</a></div></div>";
			}
